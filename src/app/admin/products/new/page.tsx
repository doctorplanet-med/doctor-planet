...
export default function AddProductPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    slug: '',
    description: '',
    costPrice: '',    // Buying price (what seller pays)
    price: '',        // Sale price (what customer pays)
    ...
    company: '',      // Brand/Company name (mainly for equipment)
  })
  ...
  // Check if we're using variant stock management
  ...
  const addSize = () => {
    if (newSize.trim() && !sizes.includes(newSize.trim())) {
      ...
      setNewSize('')
      // Initialize stock for this size in all existing colors
      if (colors.length > 0) {
  ...
  const removeSize = (size: string) => {
    ...
    // Remove this size from all color stocks
    const newStock = { ...colorSizeStock }
  ...
  const addCustomizationOption = (categoryIndex: number) => {
    ...
    if (opt && !cat.options.includes(opt)) {
      setCustomizationCategories(customizationCategories.map((c, i) =>
        i === categoryIndex
          ? { ...c, options: [...c.options, opt], newOption: '' }
          : c
      ))
    }
  }
  ...
  const setCustomizationCategoryNewOption = (categoryIndex: number, value: string) => {
    setCustomizationCategories(customizationCategories.map((c, i) =>
      i === categoryIndex ? { ...c, newOption: value } : c
    ))
  }
  ...
  const addColor = () => {
    if (newColor.trim() && !colors.includes(newColor.trim())) {
      ...
      setNewColor('')
      // Initialize stock for all sizes in this color
      if (sizes.length > 0) {
        const newStock = { ...colorSizeStock }
  ...
  // Get stock for a specific color (sum of all its sizes)
      ...
      const response = await fetch('/api/admin/products', {
        ...
        body: JSON.stringify({
          ...
          customizationCategories: hasCustomization
            ? customizationCategories.map((c, i) => ({ name: c.name, order: i, options: c.options.map((name, j) => ({ name, order: j })) }))
            : [],
        }),
      })
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-heading font-bold text-secondary-900">Add Product</h1>
            <p className="text-secondary-600 mt-1">Create a new product for your store</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            {images.length > 0 && (
              <div className="mt-6">
                ...
                <p className="text-xs text-secondary-500 mt-3">
                  First image will be used as the main product image. Drag images to reorder.
                </p>
              </div>
            )}
          ...
          >
            ...
            <p className="text-sm text-secondary-500 mb-4">
              Add unlimited tags for better SEO. Tags will be displayed at the bottom of product page.
            </p>
          ...
          >
            ...
            {hasCustomization && (
              <div className="pt-4 border-t border-secondary-200">
                <p className="text-xs text-secondary-500 mb-4">
                  Add <strong>Customization categories</strong> (e.g. Body Measurements) and <strong>options</strong> under each (e.g. Shoulder, Chest). Users will enter values (e.g. in inches) for each option.
                </p>
                ...
                {customizationCategories.length === 0 && (
                  <p className="text-xs text-secondary-500 mb-4">
                    Add a category (e.g. Body Measurements), then add options (e.g. Shoulder, Chest). Not compulsory.
                  </p>
                )}
          ...
          {colors.length > 0 && sizes.length > 0 && (
            ...
            >
              ...
              <p className="text-xs text-secondary-500 mt-4">
                Stock will be automatically calculated from these values
              </p>
            </motion.div>
          )}
        </div>
                ...
                return (
                  <div>
                    ...
                    <select
                      ...
                      className="input-field"
                    >
                      <option value="">None (all of {selectedCat.name})</option>
            ...
            <div className="space-y-4">
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  className="w-5 h-5 rounded border-secondary-300 text-primary-600 focus:ring-primary-500"
                />
                <span className="text-secondary-700">New Arrival</span>
              </label>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  className="w-5 h-5 rounded border-secondary-300 text-primary-600 focus:ring-primary-500"
                />
                <span className="text-secondary-700">Collection</span>
              </label>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  className="w-5 h-5 rounded border-secondary-300 text-primary-600 focus:ring-primary-500"
                />
                <span className="text-secondary-700">Best Selling</span>
              </label>
            </div>
          </motion.div>
